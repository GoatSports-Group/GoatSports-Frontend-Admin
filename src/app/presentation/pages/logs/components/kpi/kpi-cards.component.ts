    ...
    <div class="flex flex-col lg:flex-row items-center justify-between gap-5 w-full" 
      ...
      @if (loading || !stats) {
        @for (i of [1, 2, 3]; track i) {
          <!-- Skeleton Card -->
          <div class="bg-white border border-slate-100 rounded-xl p-4 relative overflow-hidden flex flex-col justify-between min-h-[90px] w-full lg:w-[calc(33.333%-14px)] animate-pulse">
        ...
        @for (card of statCards; track card.id) {
          ...
          <div cdkDrag 
               ...
               class="bg-white border border-slate-100 rounded-xl p-4 relative overflow-hidden flex flex-col justify-between min-h-[90px] w-full lg:w-[calc(33.333%-14px)] z-50 hover:shadow-sm transition-shadow bg-white cursor-default">
            
            <!-- Drag Placeholder matching layout width exactly -->
            <div *cdkDragPlaceholder class="border-slate-200 rounded-xl min-h-[90px] w-full lg:w-[calc(33.333%-14px)]"></div>
